	...
	)
{
	...
	ap_int<(1<<S)> mask = onebitmask ;

	ap_uint<(1 << S)> top = input.range((1 << N) - 1, (1 << N) - (1 << S)) ^ mask;
	ap_uint<1> cmp = top.and_reduce();
		...
		typename std::enable_if<GenericLZOCStageInfo<S>::is_a_power_of_2  and not(GenericLZOCStageInfo<S>::is_one)>::type* = 0)
{
	...
	ap_int<1> onebitmask = ~leading;
	ap_int<S> mask = onebitmask ;

	ap_uint<S> top = input.range(N-1, N - S) ^ mask;
	ap_uint<1> cmp = top.and_reduce();
