...
private:
 template<FPDimType T>
 auto descriptor_from_fpdim(T const & dim) {
   return FPDimRTRepr{dim.msb_weight, dim.lsb_weight, dim.is_signed};
 }

 template<BinaryExprType ET>
 auto extract_subtree() {
   auto [left_child_op_node, left_child_sym_table] = extract_subtree<typename ET::LeftType>();
   auto [right_child_op_node, right_child_sym_table] = extract_subtree<typename ET::RightType>();
 ...
 template<UnaryExprType ET>
 auto extract_subtree() {
   auto [child_op_node, child_sym_table] = extract_subtree<typename ET::ChildType>();
   OperationNode op_node{};
   op_node.op_kind.emplace(ET::operation_t::operation_kind);
 ...
 template<VariableExprType ET>
 auto extract_subtree() {
   OperationNode op_node{};
   LeafDescriptor ldescr{LeafType::VARIABLE, descriptor_from_fpdim(
       fpdim_from_integral<typename ET::type>{}
   )};
 ...
 template<ConstantExprType ET>
 auto extract_subtree() {
   OperationNode op_node{};
    LeafDescriptor leaf_descr {LeafType::CONSTANT, descriptor_from_fpdim(
       fpdim_from_integral<typename ET::type::value_type>{}
   )};
