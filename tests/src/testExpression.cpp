...
BOOST_AUTO_TEST_CASE(TestDimComputation) {
	using dim = TightFixedFormat<13, 5, -8>;
	...
	using dim2 = TightFixedFormat<13, 8, -8>;
	...
	using dim3 = FixedFormat<5, 13>;
	static_assert (dim3::EXP_CONSTRAINED == false, "Error with unconstrained FixedFormat");
	static_assert (dim3::MAX_EXP == 15, "Error with unconstrained FixedFormat");
}
